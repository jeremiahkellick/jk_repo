...
int main(void)
{
    ...
    JK_ASSERT(jk_f32_pack(jk_zero_unpacked) == 0.0f);
    JK_ASSERT(jk_f32_pack(jk_one_unpacked) == 1.0f);

    JK_ASSERT(0.0f == jk_f32_pack(jk_f32_unpack(0.0f)));
    JK_ASSERT(-0.0f == jk_f32_pack(jk_f32_unpack(-0.0f)));
    JK_ASSERT(1.0f == jk_f32_pack(jk_f32_unpack(1.0f)));
    JK_ASSERT(-1.0f == jk_f32_pack(jk_f32_unpack(-1.0f)));
    JK_ASSERT(infinity.f == jk_f32_pack(jk_f32_unpack(infinity.f)));
    JK_ASSERT(-infinity.f == jk_f32_pack(jk_f32_unpack(-infinity.f)));

    FloatUnion round_trip_nan = {.f = jk_f32_pack(jk_f32_unpack(some_nan.f))};
    JK_ASSERT(some_nan.bits == round_trip_nan.bits);
    FloatUnion nan_negative = {.f = -some_nan.f};
    FloatUnion round_trip_nan_negative = {.f = jk_f32_pack(jk_f32_unpack(nan_negative.f))};
    JK_ASSERT(nan_negative.bits == round_trip_nan_negative.bits);

    for (uint64_t i = 0; i < 10000; i++) {
        FloatUnion value = {.bits = jk_random_u64(&generator)};
        if (isnan(value.f)) {
            FloatUnion round_trip = {.f = jk_f32_pack(jk_f32_unpack(value.f))};
            JK_ASSERT(value.bits == round_trip.bits);
        } else {
            JK_ASSERT(value.f == jk_f32_pack(jk_f32_unpack(value.f)));
        }
    }
    ...
    for (uint64_t i = 0; i < 10000; i++) {
        FloatUnion value = {.bits = jk_random_u64(&generator)};
        if (!isnan(value.f)) {
            JK_ASSERT(jk_ceil_f32(value.f) == ceilf(value.f));
        }
    }
